...
class BasePipeline(torch.nn.Module):
    ...
    def preprocess_image(self, image, torch_dtype=None, device=None, pattern="B C H W", min_value=-1, max_value=1):
        ...
        # return shape: B,C,H,W , B=1 value range: [-1,1]
        image = torch.Tensor(np.array(image, dtype=np.float32)) # [H,W,C]
        image = image.to(dtype=torch_dtype or self.torch_dtype, device=device or self.device)
        image = image * ((max_value - min_value) / 255) + min_value
        image = repeat(image, f"H W C -> {pattern}", **({"B": 1} if "B" in pattern else {})) # [B,C,H,W]
        return image
    ...
    def preprocess_video(self, video, torch_dtype=None, device=None, pattern="B C T H W", min_value=-1, max_value=1):
        ...
        video = torch.stack(video, dim=pattern.index("T") // 2) # [B,3,F,H,W]
        return video
    ...
    def freeze_except2(self, pattern_list):
        """Freeze modules whose names contain any of the patterns."""
        # Handle single string input: "cross_attn" -> ["cross_attn"]
