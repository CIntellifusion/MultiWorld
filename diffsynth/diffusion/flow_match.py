...
class FlowMatchScheduler():
    ...
    def timestep_to_id(self, timestep):
        if isinstance(timestep, torch.Tensor):
            timestep = timestep.cpu()
        
        # Supporting Diffusion Forcing (per-frame timestep)
        if timestep.numel() > 1:
            timestep_flatten = timestep.flatten()
            timestep_id = torch.argmin((self.timesteps[:,None] - timestep_flatten[None,:]).abs(),dim=0)
        else:
            timestep_id = torch.argmin((self.timesteps - timestep).abs())
        return timestep_id
    
    def step(self, model_output, timestep, sample, to_final=False, **kwargs):
        timestep_id = self.timestep_to_id(timestep) 
    ...
    def add_noise(self, original_samples, noise, timestep):
        ...
        sigma = self.sigmas[timestep_id]
        # Auto-match element size with one of the original sample dimensions
        assert original_samples.shape == noise.shape
        # timestep=1000 corresponds to sigma=0 (clean data)
        # timestep=0 corresponds to sigma=1 (pure noise)
        sigma = auto_match_dim(sigma, original_samples).to(original_samples.device).to(original_samples.dtype)
