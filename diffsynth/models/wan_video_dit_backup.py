...
class DiTBlock(nn.Module):
    def __init__(self, has_image_input: bool, dim: int, num_heads: int, ffn_dim: int, eps: float = 1e-6, use_causal_cross_attention: bool = False,has_context_input: bool = False):
        ...
        if use_causal_cross_attention:
            self.cross_attn = CausalCrossAttention(dim, num_heads, eps,has_context_input=has_context_input)
        else:
...
class WanModel(torch.nn.Module):
    ...
    def forward(self,
        ...
        raise NotImplementedError("WanModel forward method is not implemented yet.")
