...
class VideoVAE_(nn.Module):
    ...
    def decode_frame(self, z_frame, scale, is_first_frame=False):
        """Decode a single latent frame incrementally, preserving feat_cache.

        Args:
            z_frame: [1, C, 1, H', W'] single latent frame
            scale: VAE scale factors
            is_first_frame: if True, clears and initializes feat_cache
        Returns:
            pixel frames tensor [1, 3, T_out, H, W] where T_out=1 (first) or 4
        """
        if is_first_frame:
            self.clear_cache()
        # Denormalize
        if isinstance(scale[0], torch.Tensor):
            scale = [s.to(dtype=z_frame.dtype, device=z_frame.device) for s in scale]
            z_frame = z_frame / scale[1].view(1, self.z_dim, 1, 1, 1) + scale[0].view(
                1, self.z_dim, 1, 1, 1)
        else:
            scale = scale.to(dtype=z_frame.dtype, device=z_frame.device)
            z_frame = z_frame / scale[1] + scale[0]
        x = self.conv2(z_frame)
        self._conv_idx = [0]
...
class WanVideoVAE(nn.Module):
    ...
    def tiled_encode(self, video, device, tile_size, tile_stride):
        ...
        for h, h_, w, w_ in tqdm(tasks, desc="VAE encoding",leave=False):
    ...
    def decode_frame(self, latent_frame, is_first_frame=False):
        """Decode single latent frame incrementally.

        Args:
            latent_frame: [1, C, 1, H', W'] single latent frame tensor
            is_first_frame: if True, resets internal caches
        Returns:
            video tensor [1, 3, T_out, H, W] clamped to [-1, 1]
        """
        latent_frame = latent_frame.to(self.model.conv2.weight.device)
        video = self.model.decode_frame(latent_frame, self.scale, is_first_frame=is_first_frame)
        return video.clamp_(-1, 1)
